...
const VidComp = ({ roomId }) => {
  ...
  const handleExistingPeers = useCallback(async (peers = []) => {
  ...
  // Accept offer
  const acceptOffer = useCallback(async ({ from, offer }) => {
  ...
  // Accept answer
  const acceptAnswer = useCallback(async ({ from, answer }) => {
    ...
    try {
      ...
      console.error("Error setting remote description (answer) from", from, err);
    }
  }, [pcs]);

  // Update ICE candidate
  const updateIceCandidate = useCallback(async (msg) => {
    const from = msg?.from;
    const candidate = msg?.candidate || msg?.ice_candidate;
  ...
  }, [socket, handleExistingPeers, handleIncomingCall, acceptOffer, acceptAnswer, updateIceCandidate, pcs]);
